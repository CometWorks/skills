...
This document provides full details about command execution on Windows for the se-dev-script skill. For most use cases, refer to Quick Start section in [CommandExecution.md](CommandExecution.md).
...
uv run search_scripts.py --help (with workdir set to the skill folder)
```
...
```cmd
C:\Users\name> uv run search_scripts.py --help
```
**Error:** `search_scripts.py` not found, wrong CWD

✅ **Solution 1:** Use workdir parameter:
```bash
uv run search_scripts.py --help (with workdir=C:\path\to\skill\folder)
```
...
```bash
uv run search_scripts.py --help (with workdir set to skill folder)
```
...
4. ✓ **Correct command format?**
   ...
   - Batch files: `.\Prepare.bat` or `Prepare.bat`
...
# Command 2
uv run search_scripts.py --help (workdir: skill_folder)
```

**Option B:** Use PowerShell with semicolons:
```powershell
cd C:\path\to\skill; .\Prepare.bat; uv run search_scripts.py --help
```
