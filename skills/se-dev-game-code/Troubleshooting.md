...
uv run search_game_code.py class usage MyEntity --limit 20
```
...
uv run search_game_code.py struct usage Vector3D -n Sandbox.Game --count

# Even more specific - use regex for exact context
uv run search_game_code.py struct usage "Vector3D" -n Sandbox.Game.Entities
```
...
uv run search_game_code.py class declaration "re:.*Physics.*" -n Sandbox.Game --count

# View limited results
uv run search_game_code.py class declaration "re:.*Physics.*" -n Sandbox.Game --limit 10
```
...
uv run search_game_code.py class declaration "mycubeblock"  # Won't match MyCubeBlock

# Case-insensitive
uv run search_game_code.py class declaration "mycubeblock" -i  # Matches MyCubeBlock
```
...
uv run search_game_code.py class parent MyTerminalBlock
