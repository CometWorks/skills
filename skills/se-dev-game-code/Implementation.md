...
Technical docs for contributors and those working on skill itself.

## Index File Formats

All CSV files located in `Data/CodeIndex/` after preparation.
...
**Note:** Only classes indexed (not structs, since structs cannot inherit from classes).
...
**Note:** Interface implementations NOT shown in tree files. Use hierarchy search commands instead.
...
- Falls back to naming convention (I-prefix) heuristic if needed
