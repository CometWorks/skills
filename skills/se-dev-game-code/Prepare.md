...
Notes:
- The actual data (decompiled sources, content files and indexes) is stored under `%USERPROFILE%\.se-dev\game-code\` on Windows and `~/.se-dev/game-code/` on Linux, exposed via the `Data` junction/symlink in this skill folder.
