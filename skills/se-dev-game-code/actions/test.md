...
Run the test script to verify game code search works correctly.
...
Each check asserts its own outcome, so reading the output is optional - the
...
As last resort, force repeating the whole preparation process by running `./clean.sh` then `./prepare.sh`
