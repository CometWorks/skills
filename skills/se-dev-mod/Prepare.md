...
2. Inform user this is one-time preparation taking about 10 seconds; building the Graphify graph adds seconds to a couple of minutes depending on how many local mods you have. The first se-dev preparation on a machine also installs Graphify (a one-time download, ~30 seconds). Set `SE_DEV_GRAPHIFY=0` to skip Graphify.
