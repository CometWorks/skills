...
This document provides more detailed command execution guidance for `se-dev-torch` skill.

## Recommended Approach

**Use skill folder's `workdir` parameter when running bash commands:**

```bash
# RECOMMENDED: use workdir parameter to run commands in skill folder
TORCH_ROOT=/path/to/Torch ./Prepare.sh (with workdir set to the skill folder)
uv run search_torch.py --help (with workdir set to the skill folder)
```
...
**DO NOT** open interactive bash shell with `busybox bash` unless specifically needed for sequence of commands.

### Option 2: PowerShell (Native Windows)

PowerShell is native Windows shell that handles backslash paths correctly:

```powershell
# PowerShell examples
cd C:\path\to\skill\folder
.\Prepare.bat
uv run search_torch.py --help
```
...
CMD is legacy Windows shell with limited features:
...
**DO NOT mix shell syntaxes.** Once you choose shell approach, use it consistently:
...
1. **Best:** Use `workdir` parameter in your bash tool
...
Ensures correct Python virtual environment used.
...
```cmd
C:\Users\name> uv run search_torch.py --help
```
**Error:** `search_torch.py` not found, wrong CWD

**Solution 1:** Use workdir parameter:
```bash
uv run search_torch.py --help (with workdir=C:\path\to\skill\folder)
```
...
```bash
TORCH_ROOT=/path/to/Torch ./Prepare.sh (with workdir set to skill folder)
```

**Step 4:** Once prepared, run commands using workdir parameter:
```bash
uv run search_torch.py --help (with workdir set to skill folder)
```
...
**If you need to run sequence of commands:**

**Option A:** Use workdir parameter for each command (recommended):
```bash
# Command 1
TORCH_ROOT=/path/to/Torch ./Prepare.sh (workdir: skill_folder)
# Command 2
uv run search_torch.py --help (workdir: skill_folder)
```

**Option B:** Use PowerShell with semicolons:
```powershell
cd C:\path\to\skill; $env:TORCH_ROOT='C:\path\to\Torch'; .\Prepare.bat; uv run search_torch.py --help
```

**Option C:** Use bash script wrapper (see `run_prepare.sh` if available)
...
3. Set `TORCH_ROOT` when you want to use existing local Torch checkout
