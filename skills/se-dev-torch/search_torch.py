#!/usr/bin/env python3
"""
Torch Source Search Tool

Search the Torch source index for classes, methods, fields, interfaces, etc.

Usage:
    python search_torch.py <category> <symbol_type> <pattern> [options]

Examples:
    python search_torch.py class declaration TorchPluginBase
    python search_torch.py method usage RegisterPluginCommands
    python search_torch.py class children Manager
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Search Torch source index")
...
def main():
    ...
    if not INDEX_DIR.exists():
        print(f"ERROR: Code index not found at {INDEX_DIR}. "
              f"Run Prepare.bat / Prepare.sh (or index_torch.py) first.", file=sys.stderr)
        sys.exit(1)
