...
3. Inform user this is one-time preparation, usually takes about 1 minute, plus roughly another minute to build the Graphify graph over the Torch checkout. The first se-dev preparation on a machine also installs Graphify (a one-time download, ~30 seconds). Set `SE_DEV_GRAPHIFY=0` to skip Graphify.
