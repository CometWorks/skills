...
uv run search_server_code.py struct declaration "re:^Vector3D$"
```
...
uv run search_server_code.py method declaration "re:^(Get|Set)"

# Match any My*Block class
uv run search_server_code.py class declaration "re:^My.*Block$"
```
...
uv run search_server_code.py field declaration "re:^[A-Z][a-zA-Z0-9]*$"

# Private fields (typically start with m_ or _)
uv run search_server_code.py field declaration "re:^[m_]"

# Interface names (start with I)
uv run search_server_code.py interface declaration "re:^I[A-Z]"

# Event handlers (typically On* methods)
uv run search_server_code.py method declaration "re:^On[A-Z]"
```
...
```bash
# Step 1: Count
uv run search_server_code.py -c class usage MyEntity
# Output: 1247
...
uv run search_server_code.py -l 10 class usage MyEntity           # First 10
uv run search_server_code.py -l 10 -o 100 class usage MyEntity    # Mid-range sample
uv run search_server_code.py -l 10 -o 1200 class usage MyEntity   # End sample
```
...
```bash
# Start broad
uv run search_server_code.py -c -n Sandbox class declaration ""
# Output: 3421

# Narrow to subnamespace
uv run search_server_code.py -c -n Sandbox.Game class declaration ""
# Output: 1856

# Further narrow
uv run search_server_code.py -c -n Sandbox.Game.Entities class declaration ""
# Output: 247

# Now fetch results
uv run search_server_code.py -l 20 -n Sandbox.Game.Entities class declaration ""
```
...
uv run search_server_code.py -n VRageMath class declaration ""

# 2. Pick interesting class, find its methods
uv run search_server_code.py -n VRageMath method declaration "" | grep Vector3D
...
uv run search_server_code.py class declaration MyTerminalBlock
...
# 3. Find usages
uv run search_server_code.py -l 20 class usage MyTerminalBlock

# 4. Find related methods
uv run search_server_code.py -n Sandbox.Game method declaration Terminal
```
...
uv run search_server_code.py -l 10 method usage GetPosition
# Read call sites

# 3. Find similar methods
uv run search_server_code.py method signature "re:^Get.*Position"
```
...
```bash
# 1. Find base class
uv run search_server_code.py class parent MyFunctionalBlock

# 2. Find siblings
uv run search_server_code.py class children MyTerminalBlock

# 3. Find what they all implement
uv run search_server_code.py class implements "re:^My.*Block$"
```
...
uv run search_server_code.py -l 20 method signature Build
# Review different overloads of Build methods

# Find methods accepting specific parameter types
uv run search_server_code.py method signature "re:.*Vector3D.*"
...
uv run search_server_code.py -n VRageMath method signature ""
# Then filter output for "static" keyword to find static methods

# Find methods with multiple pattern matches
uv run search_server_code.py method signature Get Position
...
uv run search_server_code.py -c method signature Constructor

# Examine all variations
uv run search_server_code.py method signature Constructor
...
uv run search_server_code.py class declaration Grid Physics

# Order doesn't matter - both patterns must match
uv run search_server_code.py method declaration Position Get  # Same result
```
...
uv run search_server_code.py -n VRageMath method declaration ""

# Game entities (Sandbox.Game)
uv run search_server_code.py -n Sandbox.Game.Entities class declaration ""

# Object builders (SpaceEngineers.ObjectBuilders)
uv run search_server_code.py -n SpaceEngineers.ObjectBuilders class declaration ""

# Core utilities (VRage.Library)
uv run search_server_code.py -n VRage class declaration ""
```
...
```bash
uv run search_server_code.py -l 10 -o 0 class usage MyEntity      # Window 1
uv run search_server_code.py -l 10 -o 200 class usage MyEntity    # Window 2
uv run search_server_code.py -l 10 -o 400 class usage MyEntity    # Window 3
uv run search_server_code.py -l 10 -o 600 class usage MyEntity    # Window 4
```
...
```bash
# Count total
uv run search_server_code.py -c method usage Update
# Output: 5234

# Check middle
uv run search_server_code.py -l 1 -o 2617 method usage Update

# Adjust range based on what you find
uv run search_server_code.py -l 50 -o 2600 method usage Update
```
...
uv run search_server_code.py -n Sandbox.Game class usage MyEntity
uv run search_server_code.py -n VRage class usage MyEntity
uv run search_server_code.py -n SpaceEngineers class usage MyEntity
```
