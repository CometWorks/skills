...
uv run search_server_code.py class declaration MyToolbar
```
...
```bash
uv run search_server_code.py <category> <declaration|usage> <pattern>
```
...
uv run search_server_code.py enum declaration MyRelationsBetweenPlayerAndBlock
uv run search_server_code.py method declaration GetPosition
uv run search_server_code.py field declaration AngularDamping
uv run search_server_code.py property declaration IsWorking
uv run search_server_code.py event declaration IsWorkingChanged
uv run search_server_code.py constructor declaration MyCubeBlock
```

### Find Usages

```bash
uv run search_server_code.py -l 10 class usage MyToolbar
uv run search_server_code.py -l 10 method usage GetPosition
uv run search_server_code.py -l 10 struct usage Vector3D
```
...
uv run search_server_code.py method signature "re:^Build$"

# Limit results
uv run search_server_code.py -l 10 method signature GetPosition
...
uv run search_server_code.py -l 20 class usage MyEntity
```
...
```bash
uv run search_server_code.py -l 20 class usage MyEntity
uv run search_server_code.py -l 20 -o 20 class usage MyEntity
uv run search_server_code.py -l 20 -o 40 class usage MyEntity
```

### Filter by Namespace

```bash
uv run search_server_code.py -n Sandbox.Game class declaration ""
uv run search_server_code.py -n VRageMath method declaration Add
```
...
# Explicit text match
uv run search_server_code.py class declaration "text:Toolbar"

# Regex for exact match
uv run search_server_code.py class declaration "re:^MyToolbar$"

# Regex patterns
uv run search_server_code.py class declaration "re:^My.*Block$"
uv run search_server_code.py method declaration "re:^Get.*Position$"
