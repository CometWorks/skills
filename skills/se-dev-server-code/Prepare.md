...
- The actual data (decompiled sources, content files and indexes) is stored under `%USERPROFILE%\.se-dev\server-code\` on Windows and `~/.se-dev/server-code/` on Linux/macOS, exposed via the `Data` junction/symlink in this skill folder.
