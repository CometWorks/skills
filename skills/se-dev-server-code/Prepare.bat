...
REM 1. Detect server install location (env var override takes precedence).
REM The detection is shared with VerifyServerFiles.bat.
...
REM 10. Link the server's DedicatedServer64 folder
...
if %ERRORLEVEL% EQU 2 (
    ...
    if exist Data\server_files.json del Data\server_files.json
...
REM 14b. Hash every original server file. The digests are versioned alongside the
REM decompiled sources, so diffing Data\server_files.json between two version commits
...
if exist Data\server_files.json goto skip_hashes
echo Hashing the original server files
uv run python -u hash_server_files.py --write "%SE_SERVER_ROOT%" Data
if %ERRORLEVEL% NEQ 0 goto failed
set NEED_COMMIT=1
:skip_hashes
...
set "SERVER_CODE_SOURCE_STATE=unchanged"
if "!NEED_COMMIT!"=="1" set "SERVER_CODE_SOURCE_STATE=changed"
call "%~dp0..\se-dev\GraphifyPrepare.bat" "se-dev-server-code" "%SERVER_CODE_GRAPH_ROOT%" "%SERVER_CODE_GRAPH_OUT%" "!SERVER_CODE_SOURCE_STATE!"
