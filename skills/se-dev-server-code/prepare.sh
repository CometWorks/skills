...
# Hash every original server file. The digests are versioned alongside the decompiled
# sources, so diffing Data/server_files.json between two version commits shows exactly
# which binaries a game update changed - including the ones that are neither
# assemblies nor copied into Data/Content.
if [ ! -f Data/server_files.json ]; then
    log "Hashing the original server files"
    uv run python -u hash_server_files.py --write "$SERVER_ROOT" Data
    NEED_COMMIT=1
fi
...
if [ "$NEED_COMMIT" = 1 ]; then
    SERVER_CODE_SOURCE_STATE=changed
else
    SERVER_CODE_SOURCE_STATE=unchanged
fi
se_dev_graphify_prepare "se-dev-server-code" "$SERVER_CODE_GRAPH_ROOT" "$SERVER_CODE_GRAPH_OUT" "$SERVER_CODE_SOURCE_STATE"
