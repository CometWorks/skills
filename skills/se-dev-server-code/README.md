...
This skill gives access to decompiled C# source code from the Space Engineers Dedicated Server, enabling code search and analysis for server mod, plugin, and script development.
...
- **Data/Decompiled/Content/** - Server content files (definitions, translations, blueprints)
...
- **Data/.git/** - Local Git repository tracking every decompilation; commit
  message is server version label (e.g. `1.208.015 b4`)
...
5. Copy server content into `Decompiled/Content/`
...
In `Data/CodeIndex/` after preparation:
...
Signature index includes all method types: abstract methods (no body), inline `=>` methods, block `{...}` methods. Property getters/setters are NOT indexed as signatures; they appear in field index.
...
**Note:** `method signature` is special subcommand showing full method signatures with parameters. Hierarchy subcommands (`parent`, `children`, `implements`, `implementors`) work with `class` and `interface` categories.
...
Find variable declarations with "Entity" in name in VRage namespace:
```
...
For simple lookups, direct grep is faster:
...
After finding symbol location, read source from `Data/Decompiled/`:
...
First folder in path indicates assembly (DLL) containing the code.

## Common Assemblies

| Assembly | Contains |
|----------|----------|
| `SpaceEngineersDedicated` | Server entry point, startup, configuration |
