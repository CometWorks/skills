...
The `Data` folder is a junction/symlink to the per-user persistent server-code data directory (`%USERPROFILE%\.se-dev\server-code` on Windows, `~/.se-dev/server-code` on Linux/macOS). Treat `Data/Decompiled`, `Data/Content` and `Data/CodeIndex` exactly as before.
