...
`AccessTools` is Harmony's utility class for reflection. Use to find private fields, methods, properties, types when writing patches.

## When to Use AccessTools

Use `AccessTools` when you need to:
- Access private/internal fields or properties in patch code
- Get `MethodInfo` for dynamic patching via `TargetMethod()`
- Find nested/inner types not publicly accessible
...
```csharp
// Find method by name (searches base types too)
var method = AccessTools.Method(typeof(SomeClass), "MethodName");

// Find overloaded method by parameter types
...
Use `TargetMethod()` when you can't use attributes (e.g., type is private):
...
static class MyPatch
{
    ...
    {
        // Find method inside private nested class
...
For repeated field access (e.g., in frequently called patch), create delegates once:
...
static class MyPatch
{
    ...
    {
        // Fast field access, no reflection overhead
...
- `AccessTools.Method()` - searches type **and all base types**
- `AccessTools.DeclaredMethod()` - searches **only the specified type**

Use `Declared*` variants when you need a member from a particular type level, not inherited ones:
...
```csharp
// Create instance (may be uninitialized for some types)
...
2. **Prefer `AccessTools` over raw reflection** - handles edge cases, searches base types automatically
...
5. **Check for null returns** - `AccessTools` methods return `null` if member not found rather than throwing
