...
- In development the plugin is built either of these ways:
  - By the `dotnet` command line tool or by an IDE like VSCode, JetBrains Rider or Visual Studio. Then DLL built is deployed to Pulsar's `Local` plugin folder by the `Deploy.bat` (Windows) or `Deploy.sh` (Linux) script.
  - By Pulsar using local development folder feature (needs configuring in Pulsar). Requires plugin's XML definition file to have right content.
...
- Use the `<Runtimes>` tag in plugin XML descriptor to restrict which runtimes the plugin loads on, if needed.
...
- `ClientPlugin`: Pulsar builds only source code under this folder or its subdirectories. Find plugin initialization, configuration and logging directly in this folder.
...
- `ClientPlugin/Patches`: Use this folder and namespace to host Harmony patches.
