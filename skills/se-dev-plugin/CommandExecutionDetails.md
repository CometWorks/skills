...
uv run search_plugins.py --help (with workdir set to the skill folder)
```
...
```bash
uv run search_plugins.py class declaration MyPlugin
...
```cmd
C:\Users\name> uv run search_plugins.py --help
```
**Error:** `search_plugins.py` not found, wrong CWD

✅ **Solution 1:** Use workdir parameter:
```bash
uv run search_plugins.py --help (with workdir=C:\path\to\skill\folder)
```
...
```bash
uv run search_plugins.py --help (with workdir set to skill folder)
```
...
# Command 2
uv run search_plugins.py --help (workdir: skill_folder)
```

**Option B:** Use PowerShell with semicolons:
```powershell
cd C:\path\to\skill; .\Prepare.bat; uv run search_plugins.py --help
```
