...
2. Inform user this is one time preparation taking about 1 minute, plus a few seconds up to about a minute to build the Graphify graph over the downloaded sources. The first se-dev preparation on a machine also installs Graphify (a one-time download, ~30 seconds). Set `SE_DEV_GRAPHIFY=0` to skip Graphify.
