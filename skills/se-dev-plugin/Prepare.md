...
2. Inform user this is one time preparation taking about 1 minute. The Graphify graph builds automatically when the fast Rust clustering backend is available (Python 3.12 via `uv`, provisioned automatically); over the downloaded sources it adds a few seconds up to about a minute either way. Without that backend it stays opt-in (`SE_DEV_GRAPHIFY=1`); `SE_DEV_GRAPHIFY=0` disables it. See [Prepare-time Graphify graphs](../se-dev/GraphifyPrepare.md).
